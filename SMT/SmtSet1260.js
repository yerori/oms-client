...
function screen_on_submitcomplete(mapid, result, recv_userheader, recv_code, recv_msg)
{
	...
	if(recv_userheader == "SELECT_SMT_POPUP_LIST") 
	{		
...
function btnCommonSearch_on_mouseup(objInst)
{
	...
	if (! dateST || ! dateEd) {
		UT.alert(this.screen, "MSG594", "등록일을 입력하세요"); 
		return; 
	}
	...
	TRN.gfnCommonTransactionAddSearch(this.screen , "BoardMapper.SELECT_SMT_POPUP_LIST" ,"dsSearch" , "dsList", "TR_SEARCH");	//조회만	
	// screen , "해당 로직 구분 명" , 싱크방식(true/false) , 싱크시 콜백함수 호출여부 (true/false) , 진행중보이기(true/false)
	TRN.gfnCommonTransactionRun(this.screen , "SELECT_SMT_POPUP_LIST" , true , true , false , "TR_SEARCH");	// recv_userheader 값에 select 반환   , 싱크 비동기  (screen_on_submitcomplete 자동호출됨)
}
...
function btnView_on_mouseup(objInst)
{
	...
	if( UT.isNull(boardNo)){
		UT.alert(this.screen , "MSG595" , "팝업 글을 선택하세요.");
		return;
	}
	...
	objPopupExtraData.P_DATA4 = null;
	...
	screen.loadportletpopup("view", "/SMT/SmtSet1270", "팝업 상세", false, 0, 0, 0, 1200, 551, true, true, false, objPopupExtraData);	
}
...
function btnCommonCreate_on_mouseup(objInst)
{
	...
	objPopupExtraData.P_DATA7 = "0"; //P_BOARD
	objPopupExtraData.RETURN_FUNCTION_NAME = "fnViewClosePopCallback";
	screen.loadportletpopup("New", "/SMT/SmtSet1270", "팝업 글 생성", false, 0, 0, 0, 1200, 551, true, true, false, objPopupExtraData);	
}
