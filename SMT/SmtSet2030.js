...
function fnGetTitle(){	
	...
	for(i=0; i <colLength; i++) {
		...
		// header가 싱글타입이면서 DS TYPE이 D인 행 
		else if(metaId == "LABEL00209" && dsType != "S") {
			deletedColLength++;
		}
		
		// header가 더블타입이면서 DS TYPE이 D인 행 
		else {
			var dataIndex = i - deletedColLength ;
			var idx = this.dsHeader.getdatabyname(dataIndex, "COL_IDX");
			
			// 병합되는 행 시작되는 인덱스의 홀/짝 여부 체크
			if(idx == 1) {
				conditionByNumber = this.isOddOrEven(colIndex);
				titleIndex = dataIndex;
			}
			
			var dTitle = this.dsHeader.getdatabyname(titleIndex, "TITLE");	
			...
			if (colIndex % 2 != conditionByNumber ) {  
				this.grdList.setheadertext(rowHeaderIndex, colIndex, dTitle);   
				titleIndex++;
			} 
		}
	}
