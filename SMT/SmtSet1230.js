...
{	
    ...
    if( mode == "V" ){  // 수정 불가 
       ...
       UT.gfnHrEditorStyle(this.fldBoardKind, "D");
...
function fnPageSet(){
	...
	if (objExtraData !== null) {
		...
		this.dsSearch.setdatabyname(0, "BOARD_ID", objExtraData.P_DATA1);
		this.dsSearch.setdatabyname(0, "BOARD_NO", objExtraData.P_DATA2);
		mode = objExtraData.P_DATA3;
		this.dsInsert.setdatabyname(0, "BOARD_ID", objExtraData.P_DATA1);
		this.dsInsert.setdatabyname(0, "BOARD_NO", objExtraData.P_DATA2);
		reply_flag = objExtraData.P_DATA4; 
		authScope = objExtraData.P_DATA5; 		
		this.dsInsert.setdatabyname(this.dsInsert.getpos(), "LVL",objExtraData.P_DATA6 );	
		this.dsInsert.setdatabyname(this.dsInsert.getpos(), "P_BOARD_NO",objExtraData.P_DATA7 );
		pTitle = objExtraData.P_DATA8; 
    }
    //초기화
	if (boardId !== null) {
    	this.fldBoardKind.setselectedcode(boardId); 
        UT.gfnHrEditorStyle(this.fldBoardKind, "D");
    } else{
		this.fldBoardKind.setselectedcode("B01"); //B01: 공지사항
    }
	//화면초기화
    editMode ="P";
    //사용자 변경권한 
...
function fnDsSet(){
    ...
    //data set
    UT.gfnGetCommCodes(this.dsBoardKind, "F039");  //게시판 종류
...
function btnCommonCreate_on_mouseup(objInst)
{
   //입력 항목을 reset
	...
	this.fnPageSet();
    //초기화
    this.fldBoardKind.setselectedcode("B01"); //B01: Notice
...
function fnValidateHeaderRequiredField()
{
	...
	//게시글 종류
	var boardId = this.dsInsert.getdatabyname(0,"BOARD_ID");
	if(UT.isNull(boardId) ){
	   var strfieldName = "게시글 종류";
	   UT.alert(this.screen , "MSG004" , "게시글 종류를 입력하세요.", strfieldName);
	   return false;
	}
    //내용
...
function btnCommonDelete_on_mouseup(objInst)
{
	...
	TRN.gfnCommonTransactionAddSearch(this.screen , "BoardMapper.DELETE_Board" ,"dsSearch" , "dsInsert");
    TRN.gfnCommonTransactionRun(this.screen , "DELETE_EHR_REQ_EDU" , true , true , false , "TR_DELETE");	
...
function fnSaveData( callbackflag )
{
	...
	if ( UT.isNull( boardNo)) {
		...
		TRN.gfnCommonTransactionAddSearch(this.screen , "BoardMapper.SELECT_BOARD_NO" ,"" , "dsBoardNo");	//조회만
	...
	TRN.gfnCommonTransactionAddSave(this.screen , "BoardMapper.INSERT_SMT_BOARDS" , "BoardMapper.UPDATE_SMT_BOARDS" , "", "dsInsert" );// 추가 수정 삭제 삭제
...
function fnSearch()
{
	...
	TRN.gfnCommonTransactionAddSearch(this.screen , "BoardMapper.UPDATE_SMT_BOARDS_I_COUNT" ,"dsSearch" , "");
	TRN.gfnCommonTransactionAddSearch(this.screen , "BoardMapper.SELECT_SMT_BOARDS" ,"dsSearch" , "dsInsert");	//조회만
	TRN.gfnCommonTransactionAddSearch(this.screen , "BoardMapper.SELECT_SMT_BOARDS_ATTACH" ,"dsSearch" , "dsAttachList");	//조회만
...
function screen_on_submitcomplete(mapid, result, recv_userheader, recv_code, recv_msg)
{
   ...
   if(mapid == 'TR_SEARCH')  //조회 후
	...
	if(  recv_userheader =="ATTACH"){	
		...
		objPopupAttData.P_FILE_GUBUN = "BOARD";
		objPopupAttData.P_REF_NAME = boardNo;
		objPopupAttData.P_DIR_TYPE = "BR";
		objPopupAttData.P_OU_CODE = this.dsInsert.getdatabyname(this.dsInsert.getpos(),"BOARD_ID");	
		objPopupAttData.RET_FUNC_NAME = "";
		screen.loadportletpopup("AttFile", "/FRAME/AttachFilePop", "파일첨부팝업", false, 0, 0, 0, 584, 256, true, true, false, objPopupAttData);
    }    	
}
...
function btnReply_on_mouseup(objInst)
{
	var boardId = this.dsInsert.getdatabyname( this.dsInsert.getpos(), "BOARD_ID");
	var boardNo = this.dsInsert.getdatabyname( this.dsInsert.getpos(), "BOARD_NO");
	var title = "[RE]"+ this.dsInsert.getdatabyname( this.dsInsert.getpos(), "TITLE");
	var lvl  = Number(this.dsInsert.getdatabyname( this.dsInsert.getpos(), "LVL")) +1;
	...
	objPopupExtraData.P_DATA6 = lvl;  
	objPopupExtraData.P_DATA7 = boardNo; 
	objPopupExtraData.P_DATA8 = title;   
	objPopupExtraData.RETURN_FUNCTION_NAME = "fnReplyPopCallback";
	screen.loadportletpopup("Reply", "/SMT/SmtSet1230", "게시판 답글 생성", false, 0, 0, 0, 1200, 551, true, true, false, objPopupExtraData);	
}
