...
function fnDsUserInfoSet(){
	...
	this.dsUserInfo.setdatabyname(this.dsUserInfo.getpos(),"INFO_SECURIT_YN",INI.GV_INFO_SECURIT_YN);
...
function fnAgreeConfirm(){
    ...
    TRN.gfnCommonTransactionAddUpdate(this.screen ,"systemMapper.UPDATE_AGREE_SECURIT_INFO" ,"dsUserInfo");
	TRN.gfnCommonTransactionRun(this.screen , "SecuritInfoUpdate", true, true, false, "TR_CONFIRM");
...
function screen_on_submitcomplete(mapid, result, recv_userheader, recv_code, recv_msg)
{
	...
	if(recv_userheader == "SecuritInfoUpdate"){	
		this.screen.unload();
	}
...
{
	if(this.chkAgree.getcheck()){
		UT.confirm(this.screen,"","정보보안서약서 내용에 동의하시겠습니까?","",0,"Agree_confirm");
	}
}