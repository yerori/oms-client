...
function fnPageSet(){
	...
	if (objExtraData !== null) {		
		...
		divEditName = objExtraData.P_DATA2;
		if(!UT.isNull(divEditName)){
			this.stSystemMenuTitle.settext(divEditName);
		} else {
			this.stSystemMenuTitle.settext("비고 입력");	
		}
		//W:Write R:ReadOnly
		mode = objExtraData.P_DATA3;
		if(mode == "W"){
			UT.gfnHrEditorStyle(this.edtRemark, "G");
		} else {
			UT.gfnHrEditorStyle(this.edtRemark, "D");
		}
	}				
}
