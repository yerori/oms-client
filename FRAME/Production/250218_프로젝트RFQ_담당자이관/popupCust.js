...
function fnDsSearchSet(){
	...
	this.edtCustomerName.setfocus(true);
}
...
function fnPageSet(){
	...
	if (objExtraData !== null) {
		...
		this.dsSearch.setdatabyname(0, "CUSTOMER_NAME", objExtraData.P_DATA2);
...
function btnSearch_on_mouseup(objInst){
	...
	TRN.gfnCommonTransactionAddSearch(this.screen , "OmsCommonMapper.SELECT_CUSTOMER_POP" ,"dsSearch" , "dsList");	//조회만	
...
{
	this.dsSearch.setdatabyname(0, "CUSTOMER_NAME", "");
	this.dsSearch.setdatabyname(0, "CUSTOMER_GROUP", "");
...
function edtCustomerGroup_on_keydown(objInst, keycode, bctrldown, bshiftdown, baltdown, bnumpadkey)
{
	this.fnEnter(keycode);
	return 0;
}