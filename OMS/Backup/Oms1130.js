...
{
	console.log("dsSearch", this.dsSearch.getjson());
...
function fnTransferSearch_on_click(objInst) {
	...
	// RFQ 담당자명 조회
		...
		if(btnName == "btnChargeUserPop") {
			resFuncName = "fnPopupRFQResponsePopCallback"
...
*/
function getAllSeearchData (types) {	
...
*/
function fnSearch(){	
	var userParam = this.getAllSeearchData("user");
	var rfqParam = this.getAllSeearchData("rfq");
...
function fnTransferData() {
	...
	if(retCode == "S") {
		...
		if(retCode == "S") {				
			TRN.gfnTranDataSetHandle(this.screen , this.dsUser , "ALL" , "NONE" ,  "" , "" , "TR_PROC");	
			TRN.gfnTranDataSetHandle(this.screen, this.dsProcResult, null, null, null, null, "TR_PROC");
			
			TRN.gfnCommonTransactionClear(this.screen, "TR_PROC");	
			
			TRN.gfnCommonTransactionAddSearch(this.screen , "OmsRfqMapper.CREATE_RFQ_TRANSFER" ,"dsUser", "dsProcResult");
			TRN.gfnCommonTransactionRun(this.screen , "SelectProc" , true , false , false, "TR_PROC");
			
			//결과 확인
			resCode = this.dsProcResult.getdatabyname(this.dsProcResult.getpos(), "RETCODE");
