...
function btnCustomerPop_on_click(objInst, searchName)
{
	...
	var fnName = btnName === "grdList" ? "fnListPopupCustClosePopCallback" : "fnSearchPopupCustClosePopCallback";
...
/*
* dsList 고객정보 Callback
*/
function fnListPopupCustClosePopCallback(aryHash) 
{ 
...
function btnAddRow_on_mouseup(objInst)
{
	...
	this.dsList.setdatabyname(rowCount , "ROW_STATUS" , "I");
...
function btnDelRow_on_mouseup(objInst)
{
	...
	if (projectCount > 0) {
		UT.alert(this.screen, "MSG616", "프로젝트가 등록된 RFQ는 삭제할 수 없습니다.");
		return;
	}

	GRD.gfnDeleteRow(this.screen,this.grdList);
	this.dsAuthList.deleteallrows();
...
{
	var rowStatus = this.dsList.getdatabyname(this.dsList.getpos(),"ROW_STATUS");
	...
	// 담당자 수정
		...
		var userId = this.dsList.getdatabyname(this.dsList.getpos(),"CHARGE_USER_ID");
		var userName = this.dsList.getdatabyname(this.dsList.getpos(),"CHARGE_USER_NAME");
		
		// admin 권한이 없다면, 본인이 담당한 RFQ만 수정 가능
		if(authScope != "DEPT" && INI.GV_USER_ID != userId) {
			UT.alert(this.screen , "MSG615" , "수정할 권한이 없습니다.");	//경고창 처리
			return;	
		}
		
		objPopupExtraData.clear();
		objPopupExtraData.P_DATA1 = INI.GV_OU_CODE;
		objPopupExtraData.P_DATA2 = rfqCode;	
		objPopupExtraData.P_DATA3 = userId;
		objPopupExtraData.P_DATA4 = userName;
		
		UT.gfnOpenMenuByURL("/OMS/Oms1130",objPopupExtraData);
