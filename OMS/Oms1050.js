	...
	{	
		...
		if(this.dsList.getrowcount() == 0){
			...
			UT.statMsg(this.screen , "MSG003" , "%% 건의 데이터가 조회되었습니다." , this.dsList.getrowcount());	//하단메세지 처리						
	         ...
	         var aryColumn1 = ["ESTIMATE_DATE","SALES_PRICE","MATERIAL_COST","LABOR_COST","EXPENSE_COST","PROFIT","OTHERS","CONFIRM","ATT_YN_POPUP","REMARK"];
	         var aryColumn2 = ["ESTIMATE_DATE"];
...
{
	//## 2023-12-05 Modified by Youngho.Kang
	var strConfirm = this.dsList.getdatabyname(this.dsList.getpos(), "CONFIRM");
	if (strConfirm == "Y") {
...
function fnValidForm()
{
	...
	//필수 항목 검사
	var aryDual = [ "ESTIMATE_DATE", "SALES_PRICE"];
...
function fnSearch(){		
	...
	TRN.gfnCommonTransactionAddSearch(this.screen , "OmsEstimateCostMapper.SELECT_ESTIMATE_COST" ,"dsSearch" , "dsList");	//조회만	
...
function fnSaveData(alertshow){
	...
	TRN.gfnCommonTransactionAddSave(this.screen , "OmsEstimateCostMapper.INSERT_ESTIMATE_COST" , "OmsEstimateCostMapper.UPDATE_ESTIMATE_COST" , "OmsEstimateCostMapper.DELETE_ESTIMATE_COST", "dsList" );	// 추가 수정 삭제	
	//추가후 재조회 실행						
...
function grdList_on_columnclick(objInst, nColumn, bBtnClick, nImgIndex, strImgUrl)
{
    ...
    if(this.grdList.getcolumnname(nColumn) == "ATT_YN_POPUP"){
       var estimateCostId = this.dsList.getdatabyname(this.dsList.getpos(),"ESTIMATE_COST_ID");
...
//	    this.fnSaveData(false);
		...
		objPopupAttData.P_FILE_GUBUN = "Oms1050";
...
function grdList_on_itemvaluechanged(objInst, nRow, nColumn, strPrevItemText, strItemText)
{
	...
	if(strColumn == "TARGET_COST_YN") //TARGET_COST_YN: 목표원가
	{
       var count = this.dsList.getrowcount();
	   for( var iRow = 0; iRow < count ; iRow++){
		  if(nRow != iRow ){
			 this.dsList.setdatabyname(iRow , "TARGET_COST_YN" , "N");
		  }	    			
	   }		
	}	
}
