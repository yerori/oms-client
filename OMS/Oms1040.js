	...
	{  
       ...
       if(this.dsList.getrowcount() == 0){
           ...
           if( latestYN == "Y" && detailStausCode != "D09" && detailStausCode != "D04" && auth=="W" ){ //D09:close D04:drop  W:수정권한			     	
		    ...
		    if( latestYN == "Y" && detailStausCode != "D09" && detailStausCode != "D04" && auth=="W" ){ //D09:close D04:drop  W:수정권한			     	
                var searchPlanVersion = this.dsSearch.getdatabyname(this.dsSearch.getpos(), "PLAN_VERSION");
        		var frozenPlanVersion = this.dsFrozenVersion.getdatabyname(this.dsFrozenVersion.getpos(), "FROZEN_PLAN_VERSION");

		        if (searchPlanVersion <= frozenPlanVersion){
			        this.btnUpload.setenable(false);
...
function fnSearch(){		
	...
	TRN.gfnCommonTransactionAddSearch(this.screen , "OmsProjectMapper.SELECT_FROZEN_PLAN_VERSION" ,"dsSearch" , "dsFrozenVersion");	//조회만	
...
{ 
	...
	if(aryHash["LATEST_YN"] =="Y" && aryHash["DETAIL_STATUS_CODE"] !="D09"&& aryHash["DETAIL_STATUS_CODE"]!="D04"&& aryHash["AUTH"]=="W"){ //D09:close D04:drop  R:read 권한
        ...
        this.fnGetFrozenVersion();
        var searchPlanVersion = this.dsSearch.getdatabyname(iRow, "PLAN_VERSION");
        var frozenPlanVersion = this.dsFrozenVersion.getdatabyname(this.dsFrozenVersion.getpos(), "FROZEN_PLAN_VERSION");
...
* 확정버젼
*/
function fnGetFrozenVersion()
{
	TRN.gfnTranDataSetHandle(this.screen , this.dsSearch , "ALL" , "NONE" ,  "" , "" , "TR_FROZEN_VERSION");
	TRN.gfnTranDataSetHandle(this.screen , this.dsFrozenVersion , "NONE" , "CLEAR" ,  "" , "" , "TR_FROZEN_VERSION");	

	TRN.gfnCommonTransactionClear(this.screen, "TR_FROZEN_VERSION");	//트랜젝션 데이터셋 초기화 (필수)	
	TRN.gfnCommonTransactionAddSearch(this.screen , "OmsProjectMapper.SELECT_FROZEN_PLAN_VERSION" ,"dsSearch" , "dsFrozenVersion");	//조회만
	// screen , "해당 로직 구분 명" , 싱크방식(true/false) , 싱크시 콜백함수 호출여부 (true/false) , 진행중보이기(true/false)
	TRN.gfnCommonTransactionRun(this.screen , "SELECT_FROZEN_PLAN_VERSION" , true , false , false , "TR_FROZEN_VERSION");
