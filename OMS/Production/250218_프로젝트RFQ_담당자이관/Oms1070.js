...
function screen_on_submitcomplete(mapid, result, recv_userheader, recv_code, recv_msg)
{
	...
	{		
		...
		this.daAuthList.removeallrows();
		this.grdList.setselectrow(0);
		this.grdList_on_itemclick();		
	}
...
function grdList_on_itemclick(objInst, nClickRow, nClickColumn, bBtnClick, nImgIndex, strImgUrl)
{
   ...
   // var params = "AUTH_CODE="+AUTH_CODE+" OU_CODE="+OU_CODE;
	...
	TRN.gfnTranDataSetHandle(this.screen , this.daAuthList, "NONE" , "CLEAR" ,  "" , "" , "TR_AUTH");
	TRN.gfnCommonTransactionClear(this.screen , "TR_AUTH");	//트랜젝션 데이터셋 초기화 (필수)	
	TRN.gfnCommonTransactionAddSearch(this.screen , "OmeAuthMapper.SELECT_AUTH" , "dsAuthSearch" , "daAuthList");
...
function fnValidForm()
{
	...
	//필수 항목 검사
	var aryDual = [ "PROJECT_CODE", "USER_NAME"];
...
/*    for(var i=0;i<this.dsList.getrowcount();i++){
	...
	TRN.gfnTranDataSetHandle(this.screen , this.daAuthList , "ALL" , "NONE" ,  "" , "" , "TR_SAVE_ALL");						//데이터셋 인:ALL 아웃:CLEAR 정의	
	TRN.gfnCommonTransactionClear(this.screen , "TR_SAVE_ALL");	//트랜젝션 데이터셋 초기화 (필수)
	TRN.gfnCommonTransactionAddSave(this.screen , "OmeAuthMapper.INSERT_AUTH" , "OmeAuthMapper.UPDATE_AUTH" , "OmeAuthMapper.DELETE_AUTH", "daAuthList" );	// 추가 수정 삭제	
	//추가후 재조회 실행						
...
function btnAddRow_on_mouseup(objInst)
{
	...
	var rowCount = this.daAuthList.getrowcount();

	//var row = this.dsList.getpos()+1;	
	var iRow = GRD.gfnInsertRow(this.screen,this.grdAuthList,false,rowCount);	
	this.daAuthList.setdatabyname(iRow , "OU_CODE" , ouCode);	
	this.daAuthList.setdatabyname(iRow , "PROJECT_CODE" , this.dsList.getdatabyname(sRow,"PROJECT_CODE"));
}
...
function grdAuthList_on_columnclick(objInst, nColumn, bBtnClick, nImgIndex, strImgUrl)
{
	...
	//사용자 Poupup
	if(this.grdAuthList.getrowoperation(this.daAuthList.getpos()) != XFD_ROWOP_INSERT ){
		return;
	}
...
{ 
	var iRow = this.daAuthList.getpos();
	if (UT.gfnGetAryHashCount(aryHash) != 0) {
		this.daAuthList.setdatabyname(iRow , "USER_ID" , aryHash["USER_ID"]);
		this.daAuthList.setdatabyname(iRow , "USER_NAME" ,aryHash["USER_NAME"]);
	}
}
