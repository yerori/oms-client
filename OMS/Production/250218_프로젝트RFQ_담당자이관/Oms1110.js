...
//	TRN.gfnCommonTransactionAddSearch(this.screen , "OmsRfqMapper.SELECT_RFQ" ,"dsSearch" , "dsList", "TR_SEARCH");	//조회만		
...
{ 
	...
	if (UT.gfnGetAryHashCount(aryHash) != 0) {
		...
		this.dsList.setdatabyname(iRow , "RFQ_CHARGE_NAME" ,aryHash["USER_NAME"]);
	}
}
/*
* 첨부파일 Callback
...
function fnValidForm()
{
	...
	//필수 항목 검사
	var aryDual = [ "CUSTOMER_NAME", "CAR_TYPE_NAME","PRODUCT_NAME", "RFQ_CHARGE_NAME", "RFQ_DATE"];
...
function fnSaveData(alertshow){
	...
	TRN.gfnCommonTransactionAddSave(this.screen , "OmsRfqMapper.INSERT_RFQ" , "OmsRfqMapper.UPDATE_RFQ" , "OmsRfqMapper.DELETE_RFQ", "dsList" );	// 추가 수정 삭제	
	//추가후 재조회 실행						
