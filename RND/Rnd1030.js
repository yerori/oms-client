...
function fnSearch()
{
	...
	TRN.gfnCommonTransactionAddSearch(this.screen , "RndWorkHourMgtMapper.SELECT_RND_WORK_CALENDAR" ,"dsSearch" , "dsWorkCal");	//조회만
		...
		if(this.dsWorkCal.getrowcount() == 1){
			...
			for(var i=1;i<32;i++){
			    ...
			    if(!vHeaderDate){
			        ...
			        for(var j=2;j<this.grdCalendar.getrowcount();j++){
			            this.grdCalendar_inputhour_forecolor(j,i,"DD_"+vDate);
			        }
			    }
			...
			for(var j=2;j<this.grdCalendar.getrowcount();j++){
			    this.grdCalendar_inputhour_forecolor(j,32,"ROW_TOTAL");
			}
...
입력시간 글자색 제어 : 규정시간보다 입력시간이 작으면 빨간색, 같으면 검은색, 크면 파란색
*/
function grdCalendar_inputhour_forecolor(nRow,nColumn,strColumn)
{
    var vReguralHour = parseFloat(this.dsWorkCal.getdatabyname(1,strColumn)) ;
    var vInputHour = parseFloat(this.dsWorkCal.getdatabyname(nRow,strColumn)) ;
    
    if(vReguralHour > vInputHour){
        this.grdCalendar.setitemforecolor(nRow,nColumn,factory.rgb(255,0,0));
    }
    else if(vReguralHour == vInputHour){
        this.grdCalendar.setitemforecolor(nRow,nColumn,factory.rgb(0,0,0));
    }
    else if(vReguralHour < vInputHour){
        this.grdCalendar.setitemforecolor(nRow,nColumn,factory.rgb(0,0,255));
    }
}
