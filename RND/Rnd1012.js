...
function fnPageSet(){
	...
	this.fnComboSet("Y"); // 차종 콤보박스 셋팅	
}

/*
* 콤보박스 셋팅 (차종)
*/
function fnComboSet(active_flag)
{
	// 차종
    var vParam = "GROUP_CODE=O018 ACTIVE_FLAG=" + active_flag; 	
	TRN.gfnTranDataSetHandle(this.screen, this.dsCarType, "NONE", "CLEAR", "", "", "TR_CAR_TYPE");           
	TRN.gfnCommonTransactionClear(this.screen, "TR_CAR_TYPE");                                                      
	TRN.gfnCommonTransactionAddSearch(this.screen, "RndWorkHourMapper.SELECT_PROJECT_COMMON_CODE", "", "dsCarType", vParam);   
	TRN.gfnCommonTransactionRun(this.screen, "dsCarType_SELECT_COMB", true, false, false, "TR_CAR_TYPE");    
}
...
function btnSearch_on_mouseup(objInst){
	...
	TRN.gfnCommonTransactionAddSearch(this.screen , "RndWorkHourMapper.SELECT_PROJECT_POPUP" ,"dsSearch" , "dsList");	//조회만	
