...
function fnDsSearchSet(){
	...
	UT.gfnTranceCodeSet(this.dsDocStep, "N");  	     // 빈줄추가
...
function screen_on_submitcomplete(mapid, result, recv_userheader, recv_code, recv_msg)
{
	...
	if(recv_userheader == "selectDocList")
	{		
		if(this.dsDocList.getrowcount() == 0){
			UT.alert(screen , "MSG001" , "검색 결과가 없습니다.");	//경고창 처리
			return;
		} else {
			UT.statMsg(screen , "MSG003" , "%% 건의 데이터가 조회되었습니다." , this.dsDocList.getrowcount());	//하단메세지 처리
...
function fnSearch(){		
	...
	TRN.gfnTranDataSetHandle(this.screen , this.dsDocList , "NONE" , "CLEAR" ,  "" , "" , "TR_SEARCH");	

	TRN.gfnCommonTransactionClear(this.screen, "TR_SEARCH");	//트랜젝션 데이터셋 초기화 (필수)	
	TRN.gfnCommonTransactionAddSearch(this.screen , "OmsPsoMapper.SELECT_PSO_DOCU_MSTR" ,"dsSearch" , "dsDocList");	//조회만	
	TRN.gfnCommonTransactionRun(this.screen , "selectDocList" , true , true , false , "TR_SEARCH");	// recv_userheader 값에 select 반환   , 싱크 비동기  (screen_on_submitcomplete 자동호출됨)
}
...
{
	//필수 항목 검사
	var aryDual = ["DOC_STEP" ,"DOC_CHASU" ,"DOC_DETAIL_CHASU", "CONTENTS","EFFECTIVE_START_DATE"];
...
function fnSaveData(alertshow){
	...
	var rCnt = this.dsDocList.getrowcount();
	for(var iRow=0;iRow<rCnt;iRow++){
		var ouCode = this.dsDocList.getdatabyname(iRow,"OU_CODE");
		if( UT.isNull(ouCode)){
			this.dsDocList.setdatabyname(iRow, "OU_CODE", this.dsSearch.getdatabyname(this.dsSearch.getpos(),"OU_CODE") );
		}
		var doc_id = this.dsDocList.getdatabyname(iRow,"DOC_ID");
		if( UT.isNull(doc_id)){
			...
			TRN.gfnCommonTransactionAddSearch(this.screen , "OmsPsoMapper.SELECT_PSO_DOC_ID_SEQ" ,"" , "dsDocID");	//조회만	
			TRN.gfnCommonTransactionRun(this.screen , "SELECT_PSO_DOC_ID_SEQ" , true , false , false , "TR_DOC_ID");
			var newdoc_id = this.dsDocID.getdatabyname(this.dsDocID.getpos(),"SEQ");
			this.dsDocList.setdatabyname(iRow, "DOC_ID", newdoc_id);
		}
	}
	...
	TRN.gfnTranDataSetHandle(this.screen , this.dsDocList , "ALL" , "NONE" ,  "" , "" , "TR_SAVE_ALL");						//데이터셋 인:ALL 아웃:CLEAR 정의
	
	TRN.gfnCommonTransactionClear(this.screen , "TR_SAVE_ALL");	//트랜젝션 데이터셋 초기화 (필수)
	
	TRN.gfnCommonTransactionAddSave(this.screen , "OmsPsoMapper.INSERT_PSO_DOCU_MSTR" , "OmsPsoMapper.UPDATE_PSO_DOCU_MSTR" , "OmsPsoMapper.DELETE_PSO_DOCU_MSTR" , "dsDocList" );					// 추가 수정
...
function cboDocStep_on_keydown(objInst, keycode, bctrldown, bshiftdown, baltdown, bnumpadkey)
{
	if(keycode==13){    // 엔터키값 조회 버튼 click call
		this.btnCommonSearch_on_mouseup();
	}
...
function edtMgtDept_on_keydown(objInst, keycode, bctrldown, bshiftdown, baltdown, bnumpadkey)
{
	if(keycode==13){    // 엔터키값 조회 버튼 click call
		this.btnCommonSearch_on_mouseup();
	}
...
{
	// 문서첨부 Poupup
	if(this.grdList.getcolumnname(nColumn) == "ATTFILE_POP"){
		...
		objPopupAttData.P_REF_ID = this.dsDocList.getdatabyname(this.dsDocList.getpos(),"DOC_ID");
		...
		screen.loadportletpopup("AttFileP1010", "/FRAME/AttachFilePop", "첨부파일팝업", false, 0, 0, 0, 584, 256, true, true, false, objPopupAttData);
	}
}
