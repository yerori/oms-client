...
function fnSaveData(alertshow){
	...
	TRN.gfnTranDataSetHandle(this.screen , this.dsDocMain , "ALL" , "NONE" ,  "" , "" , "TR_SAVE_ALL");						//데이터셋 인:ALL 아웃:CLEAR 정의
	TRN.gfnTranDataSetHandle(this.screen , this.dsDocDtl , "ALL" , "NONE" ,  "" , "" , "TR_SAVE_ALL");						//데이터셋 인:ALL 아웃:CLEAR 정의
...
//    }
	...
	if(this.dsDocDtl.getdatabyname(iRow, "CHG_USER_ID") == INI.GV_USER_ID) {
		...
		if (UT.gfnGetAryHashCount(aryHash) != 0) {
			//console.log("ATTACH_RESULT : " + aryHash["ATTACH_RESULT"]);
			//console.log("ATTFILE_NM : " + aryHash["ATTFILE_NM"]);
