...
function fnInvalidSearch() {
	...
	var aryDual = [ "PROJECT_CODE", "CHARGE_USER_ID", "CHARGE_EMP_NO", "TRANSFER_DATE"];
...
function fnSearch(){			
	...
	TRN.gfnCommonTransactionAddSearch(this.screen , "OmsPsoMapper.SELECT_PSO_DOCU_BY_CHARGE_USER_CHANGE" ,"dsSearch" , "dsList");
...
function edtChargeUserName_on_changed(objInst, prev_text, curr_text, event_type)
{
	...
	this.rbPrev.setbackcoloropacity(0);
	this.rbCur.setbackcoloropacity(0);
...
function edtChargeUserName_on_keydown(objInst, keycode, bctrldown, bshiftdown, baltdown, bnumpadkey)
{
	if(keycode==13){    
		...
		if(chargeName) {
			...
			this.dsSearch.setdatabyname(0, "CHARGE_USER_ID" ,'');
			this.btnCommonSearch_on_mouseup();
		}
	}
	return 0;
...
{ 	
	if (UT.gfnGetAryHashCount(aryHash) != 0) {
		...
		this.dsSearch.setdatabyname(iRow , "CHARGE_EMP_NO" , aryHash["EMP_NO"]);
		this.dsSearch.setdatabyname(iRow , "CHARGE_USER_NAME" ,aryHash["EMP_NAME"]);		
	
	}
}


function searchPop_on_click(objInst)
{
