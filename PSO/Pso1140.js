...
function fnInvalidSearch() {
	...
	var aryDual = [ "PROJECT_CODE", "CHARGE_USER_NAME","TRANSFER_DATE"];
...
function fnSearch(){			
	...
	TRN.gfnCommonTransactionAddSearch(this.screen , "OmsPsoMapper.SELECT_PM_BY_CHARGE_USER_CHANGE" ,"dsSearch" , "dsList");
...
function btnChargeUserPop_on_click(objInst, searchName){
	...
	objPopupExtraData.RETURN_FUNCTION_NAME = "fnPopupPMResponsePopCallback";
...
* PM 담당자 Callback
*/
function fnPopupPMResponsePopCallback(aryHash){
	if (UT.gfnGetAryHashCount(aryHash) != 0) {
		var iRow = this.dsSearch.getpos();
